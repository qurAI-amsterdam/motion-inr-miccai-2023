...
def compute_jacobian_loss(input_coords, output, batch_size=None, loss_mask=None):
    ...
    loss = torch.linalg.norm(loss, 1)  # L1-norm
...
def compute_balanced_jacobian_loss(input_coords, output, loss_mask=None):
    """Compute the jacobian regularization loss."""
    # NOTE; Jorg, resdet.shape is equal to batch_size
    # Compute Jacobian matrices
    if loss_mask is not None:
        jac = compute_jacobian_matrix(input_coords, output)
        jac = jac[loss_mask]
...
def compute_jacobian_matrix(input_coords, output, add_identity=True):
    ...
    # Jorg: When would we add identity or not?
    # My hunch:
    # if output is the deformation field, i.e. it is not the new position of point x-fixed in x-moving but
    # just the origin in-dependent displacement vector, then we need to add an identity matrix (ones...).
    # below we just do it for (0,0), (1, 1), and (2, 2) separately.
    # ACTUALLY: what we return here when add_identity = True is the deformation gradient (denoted mostly as F).
